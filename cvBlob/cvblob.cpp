...
void cvFilterByRatio(CvBlobs &blobs, float minRatio, float maxRatio)
{
    CvBlobs::iterator it=blobs.begin();
    while(it!=blobs.end())
    {
        CvBlob *blob=(*it).second;
        std::vector<cv::Point> * points = cvConvertChainCodesToPoints(&(blob->contour));
        ...
        float ratio = blob->area/(3.14159265*radius*radius);
...
void cvFilterByX(CvBlobs &blobs, unsigned int minX, unsigned int maxX)
{
    CvBlobs::iterator it=blobs.begin();
    while(it!=blobs.end())
    {
        CvBlob *blob=(*it).second;
        if ((blob->minx < minX) || (blob->maxx > maxX))
        {
            cvReleaseBlob(blob);
...
void cvFilterByY(CvBlobs &blobs, unsigned int minY, unsigned int maxY)
{
    CvBlobs::iterator it=blobs.begin();
    while(it!=blobs.end())
    {
        CvBlob *blob=(*it).second;
        if ((blob->miny < minY) || (blob->maxy > maxY))
        {
            cvReleaseBlob(blob);
...
CvRect cvBoundingBox(const CvBlob& blob)
{
    std::vector<cv::Point> * points = cvConvertChainCodesToPoints(&(blob.contour));
...
CvBlobs::const_iterator getNearestBlob(const CvBlobs& blobs, const CvPoint2D64f& point)
{
    float minD = FLT_MAX,d=0;
    CvBlobs::const_iterator it = blobs.begin();
    CvBlobs::const_iterator findIt = blobs.end();
    while(it != blobs.end())
    {
        CvBlob* blob = (*it).second;
        CvPoint2D64f diff;
        diff.x = blob->centroid.x - point.x;
        diff.y = blob->centroid.y - point.y;
...
CvLabel getLastLabel(const CvBlobs& blobs)
{
    CvBlobs::const_iterator it = blobs.begin();
    CvLabel label = 0;
    while(it != blobs.end())
